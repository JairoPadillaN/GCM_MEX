...
$.fn.anexGrid = function (config) {
    ...
    var anexGrid = {
        ...
        colspan: reemplazaUndefinedPor(config.colspan, ''),
        ...
        cargarData: function () {
            ...
            var parametros = {
                ...
                fechaInicio: anexGrid.fechaInicio,
                fechaFin: anexGrid.fechaFin,
                idc: anexGrid.idc,
            ...
            $.ajax({
                ...
                success: function (r) {
                    ...
                    anexGrid.sumaTotales = r.sumaTotales;                    
                    cargarData();
                },
    ...
    /* Evento nuevo para filtro de fechas */
    anexGrid.tabla.on('change', '.fechasFiltro', function (e) {
        var control = $(this);
    ...
    function cargarColumnas() {
        ...
        $(anexGrid.columnas).each(function (i, col) {
            ...
            /* id para la columna th */
            var _id_ = convierteObjetoAEstiloCss(
                reemplazaUndefinedPor(col.id, '')
            );
            /* Aneado por Pedro Glz */
            /* id_Totales para la etiqueta de ordenar, se coloca cuando se indica que se muestre el total en la columna */
            var _id_Totales = convierteObjetoAEstiloCss(
                reemplazaUndefinedPor(col.id_Totales, '')
            );

            /* Agregamos las columnas */
            var th = $('<th class="' + reemplazaUndefinedPor(col.class, '') + '" style="' + _style_ + '" id="' + _id_ + '" colspan="' + reemplazaUndefinedPor(col.colspan, '') + '"></th>');
            ...
            /* ¿Es ordenable? */
            if (col.ordenable) {
                var a = $('<a href="#" class="' + clase.columna_ordenar + '" id="' + _id_Totales + '" data-columna="' + reemplazaUndefinedPor(col.columna, '') + '"></a>');
                a.html(th.html());
            ...
            if (anexGrid.filtrable) {
                th = $('<th colspan="' + reemplazaUndefinedPor(col.colspan, '') + '" class="' + reemplazaUndefinedPor(col.class, '') + '"></th>');
    ...
    function cargarData() {
        ...
        $(anexGrid.data).each(function (i, f) {
            ...
            $(anexGrid.modelo).each(function (x, m) {
                ...
                var td = $('<td class="' + reemplazaUndefinedPor(m.class, '') + '" style="' + _style_ + '" colspan="' + reemplazaUndefinedPor(m.colspan, '') + '"></td>');
    ...
    return {
        ...
        cargarSumaTotales: function () {
            return anexGrid.sumaTotales;
        },
    };
...
function anexGrid_input(config) {
    ...
    return '<input type="' + config.type + '" style="' + config.style + '" class="form-control input-sm filtroAnexGrid ' + config.class + '" value="' + config.value + '" ' + config.attr + ' />';
}
...
function anexGrid_select(config) {
    ...
    var control = $('<select style="' + config.style + '" class="form-control input-sm filtroAnexGrid ' + config.class + '" ' + config.attr + '></select>');
