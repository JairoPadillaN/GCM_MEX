    ...
    <form id="form_ref">
        ...
        <div class="form-group col-md-6">
            <div class="form-group row">
                ...
                <div class="col-sm-9">
                    <select class="form-control rounded-0" id="marcas" name="idMarcaRefa">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-9">
                    <select class="form-control rounded-0 mi-selector" name="idPartesVenta" id="partes"
                        onblur=calcularFinanciamiento()>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-9">
                    <select class="form-control rounded-0" name="diasDePago" id="dias"
                        onchange=calcularFinanciamiento()>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-9">
                    <select name='semanasEntrega' id='semanasEntrega' class="form-control"
                        onchange=calcularFinanciamiento() onblur=calcularFinanciamiento()>
            ...
            <div class="form-group row">
                <div class="col-sm-4" style="" id="precioVenta"><b>*Monto de venta:</b>
                    {{ Form::text('precioVenta', old('precioVenta'), ['class' => 'form-control rounded-0', 'placeholder' => 'Ejemplo: 20390', 'id' => 'precioV', 'onblur' => 'calcularFinanciamiento()', 'readonly']) }}
                </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-5" style="">
                    ...
                    Si <input type="radio" name="aplicaDescuento" id="siAplica"
                        value="Si">&nbsp;&nbsp;&nbsp;&nbsp;
                    No <input type="radio" name="aplicaDescuento" id="noAplica" value="No"
                        checked>&nbsp;&nbsp;&nbsp;&nbsp;
                </div>
            </div>
            ...
            <div id="descuentos" class="alert alert-primary" role="alert" style="display:none;">
                ...
                <div class="form-group row divConDescuento" style="display:none;">
                    ...
                    <div class="col-sm-6" style="">
                        <button type="button"
                            class="btn btn-success btn-sm form-control rounded-0 agregarReparacion">
                            <span class="glyphicon glyphicon-plus-sign"></span> Agregar SKU
                        </button>
                    </div>
                </div>
...
<div id="e"></div>
<div id="reporteReparaciones_refurbished">
    @if ($cuantos >= 1)
        <div class="table-responsive">
            <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                <thead>
                    <tr>
                        <th colspan="9" style="text-align:right">
                            <font SIZE=2>Total: {{ $tipoMoneda }} ${{ $suma }}</font>
                        </th>
                        <th colspan="2"></th>
                    </tr>
                    <tr style="background-color: #C5EBFB;">
                        ...
                        <th>
                            <font SIZE=2>SKU / Descripción</font>
                        </th>
                        <th>
                                    ...
                                    @if ($equiposNYp->idEquipos == $equipoR->idEquipos)
                                        <td style="text-align:center;width:200px;">
                                            <font SIZE=2 color="#2F7DA7">
                                                <button value="{{ $equiposNYp->partida }}" class="verDetallePartida"
                ...
                <td style="text-align:center; width:350px;">
                    <font SIZE=2>
                        @if ($equiposNYp->descuento != '')
                            <strike>{{ $tipoMoneda }} $ <?php $totalff = $equiposNYp->montoEquipo * $equiposNYp->piezas;
                ...
                <td align="center">
                    <form id='' method='POST' enctype='application/x-www-form-urlencoded'
                        ...
                        <input type="hidden" value="{{ $equiposNYp->idCotizacion }}"
                            name="idCotizacionB" id="idCotizacion">
                        <input type="hidden" value="{{ $equiposNYp->idRepCotizacion }}"
                            name="idRepCotizacion">
                        @if ($estatusCotizacion == 'En revisión')
                            <button value="{{ $equiposNYp->partida }}"
                                class="modificarMontoPartidaCotizacion btn btn-info btn-sm"
                                type="button">
        ...
        <tr>
            <td colspan="9" style="text-align:right">
                <font SIZE=2><b>Total: {{ $tipoMoneda }} ${{ $suma }}</b></font>
            </td>
            <td colspan="2"></td>
        </tr>
    </tbody>
...
<script>
    ...
    $(".verDetallePartida").click(function(e) {
        ...
        $('#cargarModal').load('{{ url('detallePartidasCotizacion') }}' + '?idCotizacion=' + idCotizacion +
    ...
    $(".modificarMontoPartidaCotizacion").click(function(e) {
        ...
        var idCotizacion = $('#idCotizacion').val();
        var tipoMoneda = $('#tipoMoneda').val();
        $('#cargarModal').load('{{ url('modificarMontoPartidaCotizacion') }}' + '?idCotizacion=' +
            idCotizacion + '&partida=' + partida + '&tipoMoneda=' + tipoMoneda);
        e.preventDefault();
    });
</script>
<script>
...
<script>
    $("#marcas").change(function() {
        ...
        $("#partes").load('{{ url('comboPartes') }}' + '?r=' + Date.now() + '&idMarcaRefa=' + this.options[this
            .selectedIndex].value);
    });
    ...
    $("#partes").change(function() {
        ...
        $("#modelo").load('{{ url('modeloParte') }}' + '?r=' + Date.now() + '&idPartesVenta=' + this.options[
            this.selectedIndex].value);
        $("#descripcion").load('{{ url('descripcionParte') }}' + '?r=' + Date.now() + '&idPartesVenta=' + this
            .options[this.selectedIndex].value);
        $("#fechaActualizacion").load('{{ url('fechaActualizacionParte') }}' + '?r=' + Date.now() +
            '&idPartesVenta=' + this.options[this.selectedIndex].value);
        $("#notaInternaYskuEquivalente").load('{{ url('notaInternaYskuEquivalente') }}' + '?r=' + Date.now() +
            '&idPartesVenta=' + this.options[this.selectedIndex].value);
        $("#precioVenta").load('{{ url('precioParte') }}' + '?r=' + Date.now() + '&idPartesVenta=' + this
            .options[this.selectedIndex].value + '&tipoMoneda=' + tipoMoneda + '&cambioDolar=' +
            cambioDolar,
            function() {
                setTimeout(() => {
        ...
        $("#semanasEntrega").load('{{ url('semanasEntregaParte') }}' + '?r=' + Date.now() + '&idPartesVenta=' +
    ...
    $(".agregarReparacion").click(function() {
        ...
        if (precioV == 0 || semanas == "" || totalFin == 0 || marcas == "" || partes == "") {
            ...
            var formData = new FormData(document.getElementById("form_ref"));
            ...
            // $("#reporteReparaciones_refurbished").load('{{ url('reporteDeReparacionesAbajo') }}' + '?' + $(this).closest('form').serialize(),
            //     function(){
            //         setTimeout(() => {
            //             document.querySelector("#tipoCotizacion").value = 'refurbished';
            //             // Create a new 'change' event
    ...
    $(".borrarReparacion").click(function(e) {
        ...
        // $("#reporteReparaciones_refurbished").load('{{ url('borrarReparacion') }}' + '?' + $(this).closest('form').serialize());        
        // // $("#divPartida").load('{{ url('generarPartida') }}' + '?' + $(this).closest('form').serialize());
...
<script>
    ...
    document.querySelector("#dias").addEventListener("change", (event) => {
        ...
        console.log(document.querySelector("#piezas"))
