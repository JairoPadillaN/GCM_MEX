                    ...
                    <div id="actualizandoCosto">
                        ...
                        <div class="row" style="background-color:#F3FAF9">
                            ...
                            <div class="col-sm-6">
                                ...
                                {{Form::text('montoEquipoInicial',$datos[0]->precioVentaOriginal,['class' => 'form-control rounded-0', 'placeholder' => 'Ejemplo: 20390.50','id'=>'costoUnitarioMC','onblur'=>'costoYpiezas()'])}}
                                <br>
                            </div>
                            <div class="col-sm-6">
                                <div>*Piezas:</div>
                                {{Form::text('piezas',$datos[0]->piezas,['class' => 'form-control rounded-0', 'placeholder' => 'Ejemplo: 20390.50','id'=>'piezasMC','onblur'=>'costoYpiezas()'])}}
                                <br>
                            </div>
                            ...
                            <div class="col-sm-6">
                                ...
                                <div class="">
                                    <select name='semanasEntrega' id='semanasEntrega_mc' class="form-control" onchange=calcularFinanciamiento_mc() onblur=calcularFinanciamiento_mc()>
                            ...
                            <div class="col-sm-6">
                                <div class=""><b>Notas:</b></div>
                                <div class="">
                                    <textarea name="notas" id="" class="form-control rounded-0" rows="4" onblur=calcularFinanciamiento_mc()>{{$datos[0]->notas}}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="col-sm-6" style="">
                                ...
                                {{Form::text('montoEquipoInicial_sin_fin',$datos[0]->precioVentaOriginal * $datos[0]->piezas,['class' => 'form-control rounded-0', 'placeholder' => 'Ejemplo: 20390.50','id'=>'totalFinMC', 'readonly'])}}
                                <!-- {{Form::text('montoEquipoInicial','0',['class' => 'form-control rounded-0','id'=>'conFinanciamiento_mc','onblur'=>'calcularPorcentaje_mc()','readonly'])}} -->
                            </div>
                            <div class="col-sm-6">
                                <div style="">Total:</div>
                                {{Form::text('montoEquipo',$datos[0]->precioVentaOriginal * $datos[0]->piezas,['class' => 'form-control rounded-0','id'=>'conFinanciamiento_mc','readonly'])}}
...
<script>
    $(document).ready(function() {        
        ...
        $("#activar").click();
        setTimeout(() => {
            document.querySelector("#piezasMC").focus();
            document.querySelector("#piezasMC").blur();
        }, 500);
        console.log("ya se dio clic")
    });
    ...
    function costoYpiezas(){
        var costoUnitarioMC = $("#costoUnitarioMC").val();
        var piezasMC = $("#piezasMC").val();

        var costoUnitarioMCEquipo = parseFloat(costoUnitarioMC);
        var piezasMCEquipo = parseFloat(piezasMC);

        var calculo = costoUnitarioMCEquipo * piezasMCEquipo;
    ...
    function calcularPorcentaje_mc() {
        ...
        var piezasMC = parseFloat(piezasMC);

        var calculo = montoFinanciamiento * piezasMC;
    ...
    function calcularDescuentoMC(){
        var financiamiento = $("#conFinanciamiento_mc").val();
    ...
    $(".guardarModificacionCostoMC").click(function() {
        ...
        if (piezasMC == 0 || totalFinMC == 0 || costoUnitarioMC == 0) {
            ...
            if(costoUnitarioMC == 0){ 
                ...
                $("#piezasMC").css('border-color', 'red');
                $("#piezasMC").css('color', 'red');
            ...
            var formData = new FormData(document.getElementById("form_modifi"));

            $.ajax({
                data: formData,
                url: '{{url('guardarModificacionMontoPartidaCotizacion')}}',
                type: "POST",
                dataType: 'json',
                processData: false,
                contentType: false,
                success: function (res) {
                    setTimeout(() => {
                        ...
                        // i se edita sin mover el select de tipo de cotizacion
                        if(document.querySelector("#tipoCotizacion").value == ""){
                        ...
                        }else if (document.querySelector("#tipoCotizacion").value == "reparacion") {
            ...
            // $("#reporteReparaciones_ev").load('{{url('guardarModificacionMontoPartidaCotizacion')}}' + '?' + $(this).closest('form').serialize());
            // $("#reporteReparaciones_nuevo").load('{{url('guardarModificacionMontoPartidaCotizacion')}}' + '?' + $(this).closest('form').serialize());
            // $("#reporteReparaciones_refurbished").load('{{url('guardarModificacionMontoPartidaCotizacion')}}' + '?' + $(this).closest('form').serialize());
