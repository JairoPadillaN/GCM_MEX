                ...
                <form action="" method="POST" enctype="multipart/form-data" id="formEditarProductoOrden">
                    ...
                    <div id="actualizandoOrden">
                        ...
                        <div class="form-group">
                            ...
                            <div>
                                No. De solicitud {{ Form::radio('tipoEntradaM', '1', $consulta->idFactura !== null) }}
                                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                Compra única {{ Form::radio('tipoEntradaM', '2', $consulta->idFactura === null) }}
                            </div>
                            <div id=segunTipo>
                                @if ($opcionCombo)
                                    <div class="sub-title">Seleccionar servicio: </div>
                                    <div>
                                        <select name='idFacturaM' id='idFacturaM' class="form-control">
                                            <option value='{{ $opcionCombo[0]->idFactura }}'>
                                                {{ $opcionCombo[0]->paracombo }}</option>
                                            @foreach ($consultaOpcionesCombo as $consultaOpcionesCombo)
                                                <option value='{{ $consultaOpcionesCombo->idFactura }}'>
                                                    {{ $consultaOpcionesCombo->paracombo }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                @elseif(!$opcionCombo)
                                    <div class="sub-title">Seleccionar servicio: </div>
                                    <div>
                                        <select name='idFacturaNuevo' id='idFacturaNuevo' class="form-control" required>
                            ...
                            <div>
                                ...
                                <div>
                                    <input type="text" class="form-control" id="descripcionM" name="descripcionM"
                                        readonly value="{{ $consulta->descripcion }}">
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6">
...
<script>
    ...
    function toggleSegunTipo() {
        if ($("input[name='tipoEntradaM']:checked").val() === '1') {
            $("#segunTipo").show();
        } else {
            $("#segunTipo").hide();
        }
    ...
    $("input[name='tipoEntradaM']").change(function() {
        toggleSegunTipo();
    });

    $("input[name='tipoEntradaM']").change(function() {
        if ($(this).val() === '1') {
            $("#facturaValue").val("{{ $idFactura }}");
            $("#idFacturaNuevo").val("");
        } else if ($(this).val() === '2') {
            $("#facturaValue").val(null);
        }
    });

    $("#idFacturaNuevo").change(function() {
        var nuevoValor = $(this).val();
        $("#facturaValue").val(nuevoValor);
    });

    $("#idFacturaM").change(function() {
        var nuevoValor = $(this).val();
        $("#facturaValue").val(nuevoValor);
    });
    ...
    $(".guardarProd").click(function(e) {
        var opcionCombo = {!! json_encode($opcionCombo) !!};
        var facuraNuevoVal = $("#idFacturaNuevo").val();
        var tipoEntradaM = $("input[name='tipoEntradaM']:checked").val();
        if (tipoEntradaM === '1' && (!opcionCombo && facuraNuevoVal == "")) {
