        ...
        <table class="table table-striped table-bordered table-hover" id="dataTables-example" style="width:90%"
            ...
            <tbody>
                            ...
                            <td>
                                <div id="botonesProds">
                                    <form action="">
                                        ...
                                        <button type="button" class="btn btn-sm btn-info editarProductoOrden"
                                    ...
                                    <form action="">
                                        ...
                                        <input type="hidden" name="precioBorrar"
                                            value="{{ $compra->precioProducto }}">
                                        <input type="hidden" name="precioOrdenBorrar"
                                            value="{{ $compra->precioOrden }}">
                                        <input type="hidden" name="ivaBorrar" value="{{ $compra->ivaDinero }}">
                                        <input type="hidden" name="isrBorrar" value="{{ $compra->isrDinero }}">
                                        <input type="hidden" name="retencionBorrar"
                                            value="{{ $compra->retencionDinero }}">
                                        <input type="hidden" name="idFacturaBorrar" value="{{ $compra->idFactura }}">
                                        <input type="hidden" name="cantidadBorrar"
                                            value="{{ $compra->cantidadOrden }}">
                                        <input type="hidden" name="idPartes" value="{{ $compra->idPartesVenta }}">
                                        <input type="hidden" name="idOrden" value="{{ $compra->idOrden }}">
                                        <input type="hidden" name="idRepcotizacion"
                                            value="{{ $compra->idRepcotizacion }}">
                                        <input type="hidden" name="idRefaccionCotizacion"
                                            value="{{ $compra->idRefaccionCotizacion }}">
                                        <button type="button" class="btn btn-sm btn-danger borrarProducto"
                                            style='width:40px; height: 35px;'>
                ...
                <tr>
                    ...
                    <td colspan="2" style="text-align: right;"> <strong>{{ $sumaTotal->moneda }}
                            ${{ $sumaTotal->importe }}</strong></td>
                </tr>
                <tr>
                    <td colspan="12" style="text-align: right;">+ ISR</td>
                    <td colspan="2" style="text-align: right;"> <strong>${{ $sumaTotal->isrDineroTotal }}</strong>
                    </td>
                </tr>
                <tr>
                    <td colspan="12" style="text-align: right;">+ IVA</td>
                    <td colspan="2" style="text-align: right;"> <strong>${{ $sumaTotal->ivaDineroTotal }}</strong>
                    </td>
                </tr>
                <tr>
                    <td colspan="12" style="text-align: right;">- Retención</td>
                    <td colspan="2" style="text-align: right;">
                        <strong>${{ $sumaTotal->retencionDineroTotal }}</strong></td>
                </tr>
                <tr>
                    <td colspan="12" style="text-align: right;">Total</td>
                    <td colspan="2" style="text-align: right;"> <strong>{{ $sumaTotal->moneda }}
                            ${{ $sumaTotal->total }}</strong></td>
                </tr>
            </tbody>
        </table>
    ...
    <script>
        $(".borrarProducto").click(function() {
            $("#reporteProductos").load('{{ url('borrarProductoOrden') }}' + '?r=' + Date.now() + $(this).closest(
                'form').serialize());
            limpiarDatosSku()
        });
